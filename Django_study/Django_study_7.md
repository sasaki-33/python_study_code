...
## モデルクラス.objects.all()の値について
Human.objects.all()で値を取り出し、index.htmlでその値を{{data}}で表示すると、< QuerySet[...] >という値が表示される  
このQuerySetは、Setの派生クラスで、クエリ取得用に機能拡張したセットらしい  
表示される値は、models. pyの__str__関数の出力した内容であり、QuerySetはHumanクラスのインスタンスの集合であるといえる  

## all()のメソッド
all()で取り出す値をレコードの値で表示する場合は、「values」メソッドを使用する  
all().values()で値を取り出し、{{data}}で表示すると、レコードの値が辞書型で表示される  
また、values('キー値')でキー値を指定すると、その値のみ入手できる  
キー値も含めない場合は、「values_list」メソッドを使用すると、値のみ取得できる  
また、リストの[0],[-1]や、len(リスト)のように、レコードの最初の要素、最後の要素、取得したレコードの数を表示することができる  
* レコードの最初の要素を取得: first()
* レコードの最後の要素を取得: last()
* レコードの数の取得: count()  
